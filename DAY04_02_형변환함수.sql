...
/*
    3. 날짜 -> 문자로 변환하기
        ...
        * 날짜/시간 형식
        ...
        5) AM   : 오전 / 오후
        6) HH   : 12시각(01~12)
        7) HH24 : 24시간(00~23)           -- 5), 6) 같이 써서 오후 2시 OR 7)만 써서 14시
        8) MI   : 분(00~59)
        9) SS   : 초(00~59)
*/
...
SELECT SYSDATE,         -- '23/07/04'
       SYSTIMESTAMP     -- '23/07/04 14:47:13.451000000 +09:00'
    FROM DUAL;

-- 현재 날짜와 시간 - 형식 지정
SELECT TO_CHAR(SYSDATE,      'YYYY-MM-DD AM HH:MI:SS'),     -- 디폴트 : 날짜만, 형식지정시 : 시간도
...
SELECT *
  FROM EXAMPLE_T
 WHERE TO_DATE(DT1) = TO_DATE('23/07/04');   -- 위 코드에서 , 'YY/MM/DD' 생략 가능
-- * 날짜 비교할 땐 그냥 TO_DATE 다 넣기

...
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1000, 1, 1, '23/07/01');
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1000, 3, 2, '23/07/03');
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1001, 5, 1, '23/07/03');
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1002, 6, 2, '23/07/04');
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1003, 7, 3, '23/07/05');
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1000, 2, 5, '23/07/07');
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1003, 8, 2, '23/07/07');
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1002, 10, 2, '23/07/08');
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1001, 10, 1, '23/07/09');
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1002, 6, 4, '23/07/10');
COMMIT;
UPDATE ORDER_T SET ORDER_ID = CONCAT(TO_CHAR(ORDER_DT,'YYMMDD'), CONCAT('-', ORDER_ID));
COMMIT;
...
SELECT O.ORDER_ID AS 주문번호, B.BOOK_ID AS 책번호, B.BOOK_NAME AS 책이름
 ...
 WHERE TO_DATE(O.ORDER_DT) BETWEEN TO_DATE('23/07/05') AND TO_DATE('23/07/09')
 ORDER BY O.ORDER_ID;
...
SELECT O.ORDER_ID AS 구매번호, C.CUST_NAME AS 구매자, B.BOOK_NAME AS 책이름, O.AMOUNT*B.PRICE AS 총구매액, O.ORDER_DT AS 주문일자
  FROM CUSTOMER_T C INNER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID INNER JOIN BOOK_T B
    ON O.BOOK_ID = B.BOOK_ID
 WHERE TO_DATE(O.ORDER_DT) NOT BETWEEN TO_DATE('23/07/04') AND TO_DATE('23/07/07');
...
SELECT C.CUST_NAME AS 고객명, B.BOOK_NAME AS 책이름, O.ORDER_DT AS 주문일자
  FROM CUSTOMER_T C INNER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID INNER JOIN BOOK_T B
    ON O.BOOK_ID = B.BOOK_ID
 WHERE O.ORDER_DT = (SELECT MAX(TO_DATE(ORDER_DT))
                       FROM ORDER_T);
                       -- 0.넣으면 왜 안 됨?
...
-- 고객명  책이름       책가격
-- NULL    골프 바이블  35000
SELECT NVL(C.CUST_NAME, 'NULL') AS 고객명, B.BOOK_NAME AS 책이름, B.PRICE AS 책가격
  FROM CUSTOMER_T C FULL OUTER JOIN ORDER_T O
...
-- 고객명  구매도서수
-- 김연아  2
SELECT C.CUST_NAME AS 고객명, COUNT(O.AMOUNT) AS 구매도서수
  FROM CUSTOMER_T C INNER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID
...
-- 대한미디어 1
-- 이상미디어 2
SELECT B.PUBLISHER AS 출판사, COUNT(O.BOOK_ID) AS 판매된책수
...
-- 추신수  86000      2
-- 박세리  0          0
SELECT C.CUST_NAME AS 고객명, NVL(SUM(B.PRICE * O.AMOUNT), 0) AS 총구매액, COUNT(O.CUST_ID) AS 구매횟수
  FROM CUSTOMER_T C LEFT OUTER JOIN ORDER_T O
...
-- 장미란  62000
SELECT 고객명, 총구매액
  FROM (SELECT C.CUST_NAME AS 고객명, SUM(B.PRICE * O.AMOUNT) AS 총구매액, ROW_NUMBER() OVER(ORDER BY SUM(B.PRICE * O.AMOUNT) DESC) AS RN
          FROM CUSTOMER_T C INNER JOIN ORDER_T O
            ON C.CUST_ID = O.CUST_ID INNER JOIN BOOK_T B
            ON O.BOOK_ID = B.BOOK_ID
         GROUP BY C.CUST_NAME, C.CUST_ID)

...
-- NULL     4      골프 바이블
-- NULL     9      올림픽 이야기
SELECT NVL(O.ORDER_ID, 'NULL') AS 주문번호, B.BOOK_ID AS 책번호, B.BOOK_NAME AS 책이름
...
SELECT C.CUST_NAME AS 고객명, 
 ...
 GROUP BY C.CUST_ID, C.CUST_NAME;   -- 이름으로 그룹핑 시, ID도 같이 ! (동명이인)

-- '김연아'가 구매한 책이 몇 종인가? (피겨의 정석을 2번 주문했다면 구매한 책은 1종)
-- 무슨 고객이 무슨 책을 샀다 먼저 조회
-- 고객명  책번호
-- 김연아  5
-- 김연아  10

SELECT C.CUST_NAME AS 고객명, 
       COUNT(DISTINCT O.BOOK_ID) AS 구매도서종  -- 같은 책 번호는 중복 제거
  FROM CUSTOMER_T C INNER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID
 WHERE C.CUST_NAME = '김연아'
...
-- 추신수  86000      2
-- 박세리  0          0
SELECT C.CUST_NAME AS 고객명, NVL(SUM(B.PRICE * O.AMOUNT), 0) AS 총구매액, COUNT(O.ORDER_ID) AS 구매횟수
  FROM CUSTOMER_T C LEFT OUTER JOIN ORDER_T O
...
SELECT A.CUST_NAME AS 고객명,
  ...
  FROM (SELECT RANK() OVER(ORDER BY SUM(B.PRICE * O.AMOUNT) DESC) AS RK,
               C.CUST_NAME, SUM(B.PRICE * O.AMOUNT) AS TOTAL
          FROM CUSTOMER_T C INNER JOIN ORDER_T O
            ON C.CUST_ID = O.CUST_ID INNER JOIN BOOK_T B
            ON B.BOOK_ID = O.BOOK_ID
